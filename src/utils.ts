import type { ImageConfig, ImageConfigValue, PluginConfig, Transformer } from '../types'
...
export function create_configs(params: URLSearchParams, deliminator: string): Partial<ImageConfig>[] {
	...
	const groups = values.reduce((acc, cur) =>
		//@ts-expect-error: This reducer will take in (string | number | boolean)[] and return (string | number | boolean)[][]
		acc.flatMap((a) => cur.map((b) => [a, b].flat())),
	) as unknown as ImageConfigValue[][]
...
export async function queue_transformers(
	image: Sharp,
