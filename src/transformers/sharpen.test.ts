...
it.each([ true, 1, 10, 0 ])('applies the transform sharpen=%s', async (input) => {
    const { image } = queue_transformers(base_image.clone(), metadata, { sharpen: input as number | true }, [ transformer ])
...
it.each([ false, 'foo' ])('doesn\'t apply the transform sharpen=%s', async (input) => {
    ...
    const { image } = queue_transformers(base_image.clone(), metadata, { sharpen: input }, [ transformer ])
