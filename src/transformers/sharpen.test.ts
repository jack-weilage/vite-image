...
it.each([ true, 1, 10, 0 ] satisfies (number | true)[])('applies the transform sharpen=%s', async (input) => {
    const { image } = await queue_transformers(base_image.clone(), { sharpen: input }, [ transformer ])
