...
it.each([ true, 1, 10, 0 ])('applies the transform gamma=%s', async (input) => {
    const { image } = queue_transformers(base_image.clone(), metadata, { gamma: input as number | true }, [ transformer ])
...
it.each([ false, 'foo' ])('doesn\'t apply the transform gamma=%s', async (input) => {
    ...
    const { image } = queue_transformers(base_image.clone(), metadata, { gamma: input }, [ transformer ])
