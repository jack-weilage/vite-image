...
it.each([ true, 1, 10, 0 ])('applies the transform blur=%s', async (input) => {
    const { image } = queue_transformers(base_image.clone(), metadata, { blur: input as number | true }, [ transformer ])
...
it.each([ false, 'foo' ])('doesn\'t apply the transform blur=%s', async (input) => {
    ...
    const { image } = queue_transformers(base_image.clone(), metadata, { blur: input }, [ transformer ])
