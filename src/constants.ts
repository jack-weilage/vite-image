...
export const CONFIG_SCHEMA = z
	.object({
		include: z
			.string({ invalid_type_error: 'config.include must be a glob pattern string' })
		...
		transformers: z
			.array(
				z.object({
					name: z.string(),
					matcher: z.function().returns(z.union([z.boolean(), z.promise(z.boolean())])),
					transform: z
						.function()
						.returns(z.union([z.unknown(), z.promise(z.unknown())]).transform((val) => val as Sharp)),
				}),
			)
			.default([]),
		default_exports: z
			.array(z.string({ invalid_type_error: 'config.default_exports[n] must be a string' }), {
				invalid_type_error: 'config.default_exports must be an array',
			})
			.default(['src', 'aspect', 'width', 'height', 'format']),
	})
	.strict()
